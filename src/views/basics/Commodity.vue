  ...
  <div>
    <div style="display: flex">
      ...
      <a-input-search
          ...
          @search="onSearch"
      />
      <a-button style=" float:right" size="large" type="danger" @click="loadTableData">
        重置
      </a-button>
    </div>
    <a-table :loading="loading" :columns="columns" :data-source="data" rowKey="id">
      <a slot="name" slot-scope="text">{{ text }}</a>
      <span slot="customTitle"> 货物名称</span>
      <span slot="action" slot-scope="text, record, index">
        <a-button @click="handleUpdate(record)" type="link"> 更新</a-button>
      <a-divider type="vertical"/>
         <a-button @click="handleDelete(record,index)" type="link">删除</a-button>
    </span>
    </a-table>
    ...
    >
      <a-form-model ref="ruleForm" :model="commodity">
        ...
        <a-form-model-item label="货物价格" prop="price">
          <a-input-number id="input" v-model="commodity.price" :min="1"/>
        </a-form-model-item>
        <a-form-model-item label="货物信息" prop="description">
